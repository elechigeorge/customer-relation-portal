...
const protect_manager = asyncHandler(async (req, res, next) => {
...
try {
  ...
  const authManager = await Manager.findOne({_id: decoded.id});


  // SET LOGGED IN USER DETAILS TO REQUEST OBJECT
  req.admin = authManager;
